...
public class FlatColorButton extends WButton {
    ...
    public FlatColorButton(Component text) {
        super(text);
    }

    public FlatColorButton(Icon icon, Component text) {
        super(icon, text);
    }
    ...
    public FlatColorButton(Component text, int regularColor, int hoveredColor, int disabledColor) {
        super(text);
        this.regularColor = regularColor;
        this.hoveredColor = hoveredColor;
        this.disabledColor = disabledColor;
    ...
    public void paint(GuiGraphics context, int x, int y, int mouseX, int mouseY) {
        ...
        if (label != null) {
            ...
            ScreenDrawing.drawStringWithShadow(context, label.getVisualOrderText(), alignment, x + xOffset, y + ((height - 8) / 2), width, color); //LibGuiClient.config.darkMode ? darkmodeColor : color);
        }
    }
